module.exports = function (dizquetv, $timeout) {
    return {
        restrict: 'E',
        templateUrl: 'templates/jellyfin-server-edit.html',
        replace: true,
        scope: {
            state: '=state',
            _onFinish: '=onFinish',
        ...
        link: function (scope) {
            ...
            scope.onSave = async () => {
                try {
                    ...
                    await dizquetv.updateJellyfinServer(scope.state.server);
                    scope.state.modified = false;
            ...
            scope.onDelete = async () => {
                try {
                    let channelReport = await dizquetv.removeJellyfinServer(scope.state.server.name);
